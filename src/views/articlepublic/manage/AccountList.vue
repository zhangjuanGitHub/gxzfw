      ...
      <div class="account-num">
        ...
        <div v-if="!isDelete">
          <el-button @click="addAccount"
                     size="small"
                     type="primary">添加授权</el-button>
          <el-button @click="deleteAccount"
                     size="small"
                     type="danger">删除授权</el-button>
        </div>
      </div>
...
<script>
import ScrollData from '@/components/Pagination'
      ...
      this.$http.post(this.$api.assessmentAccountList, this.ruleForm)
        .then(res => {
          if (res.data.data.content.length) {
            this.accountList.push(...res.data.data.content)
