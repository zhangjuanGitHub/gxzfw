    ...
    <div class="hotnews-box">
      <el-row :gutter="10">
        ...
        <el-col :xs="21"
          ...
          <div class="tabs-card">
            <div class="card-name"
                 :class="{ 'is-active': activeName==='LocalArticleWx'}"
                 @click="getData('LocalArticleWx')">
              <span>本地稿件</span>
            </div>
            <div class="card-name"
                 :class="{ 'is-active': activeName==='MaterialWxlocal'}"
                 @click="getData('MaterialWx',0)">
              <span>本地素材库</span>
            </div>
            <div class="card-name"
                 :class="{ 'is-active': activeName==='MaterialWx'}"
                 @click="getData('MaterialWx',1)">
              <span>微信素材库</span>
            </div>
            <div class="card-name"
                 :class="{ 'is-active': activeName==='PublicRecordWx'}"
                 @click="getData('PublicRecordWx')">
              <span>发布记录</span>
            </div>
          </div>
...
export default {
  ...
  methods: {
    getData (url, index) {
      if (index === 0) {
        ...
        this.activeName = 'MaterialWxlocal'
      } else if (index === 1) {
        this.$router.push({ name: url, query: { showLocal: false } })
        this.activeName = 'MaterialWx'
      } else {
  ...
  created () {
    if (this.$route.query.showLocal) {
      JSON.parse(this.$route.query.showLocal) ? this.activeName = 'MaterialWxlocal' : this.activeName = 'MaterialWx'
    } else {
