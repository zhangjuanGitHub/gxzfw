<template>
  <div>
    ...
    <el-table ref="multipleTable"
      ...
      <el-table-column label="站内沟通"
                       show-overflow-tooltip>
        <template slot-scope="scope">
          <div class="connect"
               @click="connect(scope.row, scope.$index)">发起沟通</div>
        </template>
      </el-table-column>
    </el-table>
...
export default {
  ...
  methods: {
    ...
    connect (row, index) {
      console.log(row)
      this.$router.push({ name: 'Station', query: { id: row.id, name: row.user_name, chatId: this.chatId } })
    },
    // 获取列表
    userList () {
      this.dataLess = false
    ...
    // 跳转添加绑定
    addbindings () {
      this.$router.push({ name: 'Binding1', query: { type: this.params.type } })
    }
  },
  created () {
    this.userList()
