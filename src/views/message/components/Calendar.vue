  ...
  <div class="calendar-wrapper">
    ...
    <div class="calendar-inner">
        <div class="calendar-item" v-for="(item, index) of calendarList" :key="index"
            :class="[item.disable ? 'disabled' : '']" @click="dataClick(item, index)">
           <span class="checked" v-if="checked">{{ item.date }}</span>
...
export default {
  ...
  //   let showDate = `${year}-${this.pad(month + 1)}-${this.pad(date)}`
  //   let today = this.calendarList.filter(item => item.value === showDate)
  ...
  methods: {
    ...
    correctCurrent () {
      ...
      // let value = this.stringify(instance)
    ...
    dataClick (item, index) {
      let calendar = document.querySelectorAll('.calendar-item')
      if (!item.disable) {
        calendar.forEach((i, n) => {
          if (i.className === 'calendar-item disabled') {
            i.className = 'calendar-item disabled'
          } else {
            i.className = 'calendar-item'
          }
        })
        calendar[index].className = 'calendar-item checked'
      }
    }
  }
}
