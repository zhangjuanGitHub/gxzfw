<template>
  <div class="table-side">
    ...
    <el-table startDate
              ...
              class="message-list"
              style="width: 100%"
              @filter-change="checkFalseChangeInit">
      ...
      <el-table-column prop="checkStatus"
        ...
        <template slot-scope="scope">
          ...
          <p v-if="scope.row.checkStatus === 2"
             ...
             @click="openDetail(scope.row, scope.$index)">
            审核未通过
          </p>
          ...
          <p v-if="scope.row.checkStatus === 41"
             class="checkStatus color-greed"
             @click="openDetail(scope.row, scope.$index)">校对完成</p>
    ...
    <div class="detail-dialog status-list">
      <el-dialog title="流程记录"
        ...
        <el-table :data="detailList"
          ...
          <el-table-column property="checkReason"
                           label="未通过理由">
            <template slot-scope="scope">
              <p v-if="!scope.row.checkReason">----</p>
              <p v-else>{{scope.row.checkReason}}</p>
            </template>
          </el-table-column>
        ...
        <div class="easy-msg">
          <div class="easy-btn">
            <el-button type="primary"
                       ...
                       @click="changeLog">流程图</el-button>
            <el-button type="primary"
                       size="small"
                       :disabled="!showProcess"
                       @click="changeProcess">日志</el-button>
          </div>
          <div class="easy-text">
...
export default {
  ...
  methods: {
    ...
    // 获取稿件列表
    getCompanyLists () {
      let api = this.$api.listInside
      this.$http.post(api, this.ruleForm)
        .then(res => {
          this.tableData = res.data.data.content
          this.total = res.data.data.totalElements
          this.$emit('CompanyArticle', res.data.data)
        })
        .catch(() => {
          this.tableData = []
        })
    ...
    zero (res) {
      res = res > 0 ? (res > 9 ? res : '0' + res) : '00'
      return res
    },
    // 任务倒计时方法
    showtime () {
      var nowtime = new Date() // 获取当前时间
      var endtime = new Date('2020/10/26 10:50:00') // 定义结束时间
      var lefttime = endtime.getTime() - nowtime.getTime() // 距离结束时间的毫秒数
      var leftd = Math.floor(lefttime / (1000 * 60 * 60 * 24)) // 计算天数
      var lefth = this.zero(Math.floor(lefttime / (1000 * 60 * 60) % 24)) // 计算小时数
      var leftm = this.zero(Math.floor(lefttime / (1000 * 60) % 60)) // 计算分钟数
      var lefts = this.zero(Math.floor(lefttime / 1000 % 60)) // 计算秒数
      this.finishDate = lefttime < 0 ? '00:00:00' : (leftd ? (leftd + '天' + lefth + ':' + leftm + ':' + lefts) : (lefth + ':' + leftm + ':' + lefts))
      if (lefttime < 0) {
        clearInterval(this.timer)
      }
    },
