      ...
      <div class="account-list">
        <div class="list-box"
          ...
          <el-image :src="require('@/assets/images/empty-data.png')"
                    :fit="'fill'"></el-image>
          <p><router-link class="text-click" :to="{name:'UserList',query:{dis:this.form.cid,tabs:'douyin'}}">暂无数据,请前往添加</router-link></p>
        </div>
...
export default {
  ...
  methods: {
    ...
    routeBank (item, index) {
      this.$router.push({ name: 'AccountDetail', query: { id: item.id, type: 'douyin' } })
    },
    // 获取文件夹
    getFolder () {
