          ...
          <div class="list-right">
            <div class="list-user">
              ...
              <span @click="getDetail(item.account,index)"
                    class="user-name">{{item.account.nickname}}</span>
            </div>
            <div class="list-title">
              <h2 v-html="item.article.title"
                  @click="Websock(item.article,index)" class="lin-clamp-1"></h2>
            </div>
...
export default {
  ...
  methods: {
    ...
    getRouterData () {
      ...
      this.dataNull = false
      this.dataLess = false
      this.$http.post(this.$api.wxNewsList, this.form)
        .then(res => {
    ...
    getDetail (item, index) {
      this.$router.push({ name: 'CenterDetail', query: { id: item.wxid, type: 'wx', come: this.$route.query.dis } })
    },
    // 获取地区列表
    getScreenlist () {
      let row = {
