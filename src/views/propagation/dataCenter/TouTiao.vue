    ...
    <div class="data-out-list">
      <div class="data-center-list">
        <div class="data-page"
          ...
          <!-- 头条 -->
          <div :class="item.article.image_url?'list-right':'list-center'">
            <div class="list-user">
              ...
              <span @click="getDetail(item.account,index)"
                    class="user-name">{{item.account.name}}</span>
            </div>
...
export default {
  ...
  methods: {
    ...
    getRouterData () {
      ...
      this.dataNull = false
      this.dataLess = false
      this.$http.post(this.$api.ttNewsList, this.form)
        .then(res => {
    ...
    getDetail (item, index) {
      this.$router.push({ name: 'CenterDetail', query: { id: item.id, type: 'toutiao', come: this.$route.query.dis } })
    },
    // 获取地区列表
    getScreenlist () {
      let row = {
