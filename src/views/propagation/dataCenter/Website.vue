    ...
    <div class="data-search">
      <div class="data-center-header data-center-box">
        <el-form ref="form"
          ...
          <el-form-item prop="keyword" class="keyword"
                        label="关键字">
...
export default {
  ...
  methods: {
    ...
    getRouterData () {
      ...
      this.dataNull = false
      this.dataLess = false
      this.$http.post(this.$api.wzNewsList, this.form)
        .then(res => {
    ...
    getDetail (item, index) {
      this.$router.push({ name: 'CenterDetail', query: { id: item.id, type: 'website', come: this.$route.query.dis } })
    },
    // 获取地区列表
    getScreenlist () {
      let row = {
