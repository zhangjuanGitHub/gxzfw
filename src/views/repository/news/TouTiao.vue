  ...
  <div class="content-box new-media">
        ...
        <div class="right-box">
          <div class="department-detail">
            ...
            <div class="department-operation">
              <div class="dep-share-box">
                <el-popover placement="top"
                            width="160"
                            v-model="item.syncAuxiliary">
                  <p>{{item.isSync === 1 ? '是否取消共享？' : '是否共享到公共素材库？'}}</p>
                  <div style="text-align: right; margin-top: 10px">
                    <el-button size="mini" type="text"  @click="ifShowShare(item, index, false)">取消</el-button>
                    <el-button type="primary" size="mini" @click="confirmToShare(item, index)">确定</el-button>
                  </div>
                </el-popover>
              </div>
              <el-tooltip class="item" effect="dark" content="分享" placement="bottom">
                <span class="el-icon-share" @click="ifShowShare(item, index, true)"></span>
              </el-tooltip>
    ...
    <div class="text-center"
      ...
      <img :src="require('@/assets/images/empty-data.png')"
           alt="">
      <p class="text-click" @click="$router.push({ name: 'NewMediaTout' })">暂无数据，请前往添加</p>
    </div>
    ...
    <v-diaing :isDiaingShow="deleteVisible"
...
export default {
  ...
  methods: {
    // 分享
    ifShowShare (item, index, isShow) {
      item.syncAuxiliary = isShow
    },
    confirmToShare (item, index) {
      item.syncAuxiliary = false
      if (item.isSync === 1) {
        this.ifShare = 0
      } else {
        this.ifShare = 1
      ...
      this.$http.post(this.$api.updateNewscloud, form)
        .then(res => {
    ...
    // 编辑
    editList (item, index) {
      let toEdit = this.$router.resolve({ name: 'Contribute', query: { id: item.id, isArchive: '6', article: 'td' } })
        ...
        .then(res => {
          if (res.data.data.length) {
            ...
            this.wxList.forEach(item => {
              // item.originalContent = item.originalContent.replace(/\\u/gi, '%u')
              // item.originalContent = item.originalContent.replace(/\\/gi, '')
              // console.log(item.originalContent)
            })
          } else {
            this.dataNull = true
            this.wxList = []
