              ...
              <div class="my-image-list"
                ...
                <el-tooltip content="编辑" placement="top">
                    <i @click="editGroupName('edit', item, index, item.isEditGroupName)"
                        v-if="!item.isEdit  && item.isDefault !== 1 && item.isShare !== 1"
...
export default {
  ...
  methods: {
    ...
    // 下载
    downloadFile (id) {
      // window.open(`${this.$api.downloadFile}?groupId=${id}&fileId=`)
    ...
    imageUpload (item) {
      this.$router.push({ name: 'PublicCreateImage', query: { item: JSON.stringify(item) } })
    },
    newsFile () {
      this.$router.push({ name: 'PublicCreateImage' })
    },
    // 获取图片列表
    getImageList () {
  ...
  computed: {
    ...
    isSubmit () {
      ...
      if ((form.groupName && form.calcFileName.length >= 1) || (form.time && form.time.length >= 1) || form.userName) {
        boolean = false
      }
      return boolean
    }
...
.Picturebox .card-operation i:nth-of-type(2) {
  color: #c86cdf;
}
.Picturebox .details-span {
  color: #1783ff;
