<template>
  <div class="videobox min-height Picturebox">
...
export default {
  ...
  methods: {
    ...
    submitForm () {
      ...
      // 输入了关键字但没勾选类别和图片 || 没输入关键字但勾选了类别和图片
      // if ((form.calcFileName.length <= 0 && form.groupName !== '') || (form.calcFileName.length >= 0 && !form.groupName)) {
      //   this.$message.error('请输入勾选类别名称或图片名称！')
      //   return
      // }
    ...
    videoUpload (item) {
      this.$router.push({ name: 'PersonUploadVideo', query: { item: JSON.stringify(item) } })
    },
    newsFile () {
      this.$router.push({ name: 'PersonUploadVideo' })
    },
    // 获取列表
    getVideoList () {
...
.videobox-seach .el-date-picker .el-date-editor {
  width: 200px;
}
.videobox {
  min-height: 500px;
